...
public final class PhoneHomeLogger {
    ...
    public void app(final PhoneHomeLogEvent event) {
        if (logToLogcat(Log.DEBUG))
    ...
    public void flush() {
        if (logToSink())
            PhoneHomeLogQueue.getInstance().flushQueue();
    }
    ...
    private void maybeSendLog(PhoneHomeLogEvent event) {
        if (logToSink())
            PhoneHomeLogQueue.getInstance().enqueue(new Date(), event);
    }
